...
function evaluateSpin(reels: ReadonlyArray<typeof slotSymbols[number]>) {
  ...
  if (same) return { multiplier: first.payout, description: `Triple ${first.name}`, jackpot: first.name === 'Seven' };
  if (twoSame) return { multiplier: 1.5, description: 'Two matching symbols', jackpot: false };
  return { multiplier: 0, description: 'No win', jackpot: false };
}

export async function playSlot(discordId: string, guildId: string, betAmount: bigint): Promise<SlotResult & { jackpot: boolean }> {
  ...
  if (activeSpins.has(lockKey)) throw new AppError('Your previous spin is still being processed. Please wait a moment.');
  activeSpins.add(lockKey);

  try {
    const { user, config } = await getOrCreateUser(discordId, guildId);
    if (betAmount <= 0n) throw new AppError('Your bet must be greater than **0** 🪙.');
    if (user.wallet < betAmount) {
      const missing = betAmount - user.wallet;
      throw new AppError(`╭─〔 💳 WALLET CHECK 〕─╮\n〢 **Balance:** ${formatCurrency(user.wallet, config.currencyEmoji)}\n〢 **Required:** ${formatCurrency(betAmount, config.currencyEmoji)}\n〢 **Missing:** ${formatCurrency(missing, config.currencyEmoji)}\n〢 You need **${formatCurrency(missing, config.currencyEmoji)}** more to spin.\n╰─〔 📈 Earn with /work • /daily • /beg 〕─╯`);
    }

    const reels = [chooseSymbol(), chooseSymbol(), chooseSymbol()];
    const evaluation = evaluateSpin(reels);
    const grossReturn = evaluation.multiplier > 0 ? (betAmount * BigInt(Math.round(evaluation.multiplier * 100))) / 100n : 0n;
    const netAmount = evaluation.multiplier > 0 ? grossReturn - betAmount : -betAmount;

    const updatedUser = await adjustBalance(user.id, { walletDelta: netAmount }, {
      source: 'slot',
      amount: netAmount,
    ...
    return {
      ...
      message: evaluation.multiplier > 0
        ? `${evaluation.jackpot ? '💎 JACKPOT! ' : '🎉 '}You won ${formatCurrency(netAmount, config.currencyEmoji)} — no tax deducted.`
