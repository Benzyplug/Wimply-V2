...
// Temporary release setting: keep Aviator deterministic at 100× until the game design is changed.
export const AVIATOR_CRASH_POINT = 100;

export function getAviatorMultiplier(session: AviatorSession, now = Date.now()) {
  const elapsed = Math.max(0, now - session.startedAt);
  const live = 1 + elapsed / 5200;
...
export async function startAviator(discordId: string, guildId: string, amountString: string) {
  ...
  const session: AviatorSession = { id: randomBytes(8).toString('hex'), userId: updated.id, discordId, guildId, bet: amount, crashAt: AVIATOR_CRASH_POINT, multiplier: 1, startedAt: Date.now(), cashedOut: false };
