...
const THEMES: Record<string, { bg: [number,number,number]; panel: [number,number,number]; accent: [number,number,number] }> = {
  midnight: { bg:[10,16,30], panel:[18,28,48], accent:[88,101,242] },
  ocean: { bg:[5,25,42], panel:[9,43,65], accent:[45,180,210] },
  royal: { bg:[25,10,38], panel:[45,18,64], accent:[170,90,255] },
  neon: { bg:[8,20,18], panel:[12,42,34], accent:[50,220,150] },
  gold: { bg:[35,25,8], panel:[58,42,13], accent:[235,185,60] }
};
...
const command: Command = {
  data: new SlashCommandBuilder()
    .setName('profile')
    .setDescription('View a Wimply economy profile')
    .addUserOption(o => o.setName('user').setDescription('User to view').setRequired(false))
    .addStringOption(o => o.setName('background').setDescription('Set your profile background').setRequired(false)
      .addChoices({name:'Midnight',value:'midnight'},{name:'Ocean',value:'ocean'},{name:'Royal',value:'royal'},{name:'Neon',value:'neon'},{name:'Gold',value:'gold'})),
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (selectedBackground && target.id !== interaction.user.id) {
      await interaction.editReply({ content: 'You can only change your own profile background. Use `/profile user:<member>` to view another profile.' });
      return;
    }

    let { user, config } = await getOrCreateUser(target.id, interaction.guildId);
    if (selectedBackground && selectedBackground !== user.profileBackground) {
      user = await prisma.user.update({ where: { id: user.id }, data: { profileBackground: selectedBackground } });
    }

    const inventory = await getInventory(user);
    const transactions = await prisma.economyTransaction.findMany({
      where: { userId: user.id, source: { in: ['blackjack','coinflip','dice','higherlower','mines','slot','snailgarden'] } },
    ...
    for (const transaction of transactions) {
      const stats = gameStats[transaction.source];
      if (!stats) continue;
      if (transaction.amount < 0n) stats.played += 1;
      if (transaction.amount > 0n) stats.wins += 1;
    ...
    const badges = [...new Set([
      ...user.badges,
      ...(user.level >= 5 ? ['🏅 Rising Star'] : []),
      ...(netWorth >= 10_000n ? ['💰 Money Maker'] : []),
      ...(totalWins >= 10 ? ['🎰 Lucky Winner'] : []),
      ...(totalPlayed >= 50 ? ['🎮 Game Regular'] : [])
    ])];
    const achievements = [
      user.level >= 2 ? '🌟 Reached Level 2' : null,
      netWorth >= 100_000n ? '💎 100K Net Worth' : null,
      totalWins >= 25 ? '🏆 25 Game Wins' : null,
      totalPlayed >= 100 ? '🎮 100 Games Played' : null,
      user.xp >= 1_000 ? '📊 1K XP Earned' : null
    ...
    const topGames = gameSources
      .filter(source => gameStats[source].played > 0)
      .sort((a,b) => gameStats[b].played - gameStats[a].played)
      .slice(0, 4)
      .map(source => `${source === 'higherlower' ? 'Higher/Lower' : source[0].toUpperCase()+source.slice(1)}: **${gameStats[source].played}** played • **${gameStats[source].wins}** wins`)
      .join('\n') || 'No casino games recorded yet.';

    const embed = new EmbedBuilder()
      .setColor(0x5865f2)
      .setTitle(STYLE.title('👤',`${target.username}'s Profile`))
      .setDescription(`〢 **Level ${user.level}** • **${Math.floor(progress)}% XP**\n〢 ${formatCurrency(user.wallet,config.currencyEmoji)} wallet • ${formatCurrency(user.bank,config.currencyEmoji)} bank\n〢 💰 Net worth: **${formatCurrency(netWorth,config.currencyEmoji)}**\n\n🏅 **Badges**\n${badges.length ? badges.join(' • ') : 'No badges yet.'}\n\n🏆 **Achievements**\n${achievements.length ? achievements.join('\n') : 'No achievements unlocked yet.'}`)
      ...
      .setImage('attachment://wimply-profile.png')
