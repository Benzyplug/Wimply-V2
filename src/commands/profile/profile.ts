...
function cardImage(name: string, level: number, xp: number, xpForNext: number, wallet: string, bank: string, inventory: number) {
  const width = 900, height = 500; const pixels = Buffer.alloc(width * height * 4); const set = (x: number, y: number, r: number, g: number, b: number, a = 255) => { if (x < 0 || y < 0 || x >= width || y >= height) return; const i = (y * width + x) * 4; pixels[i]=r; pixels[i+1]=g; pixels[i+2]=b; pixels[i+3]=a; };
  ...
  const text = (value: string, x: number, y: number, scale: number, c: [number,number,number]) => { let cx=x; for (const char of value.toUpperCase().slice(0, 32)) { const glyph=FONT[char] ?? FONT[' ']; for (let gy=0; gy<7; gy++) for (let gx=0; gx<5; gx++) if (glyph[gy][gx] === '1') rect(cx+gx*scale,y+gy*scale,scale,scale,c); cx += 6*scale; } };
  rect(0,0,width,height,[10,16,30]); rect(24,24,width-48,height-48,[18,28,48]); rect(24,24,width-48,8,[88,101,242]);
  text('WIMPLY PROFILE',55,55,5,[240,244,255]); text(`LEVEL ${level}`,55,115,5,[120,160,255]); text(name,55,165,4,[255,255,255]);
  text('WALLET',55,245,3,[150,165,190]); text(wallet,55,275,4,[255,255,255]); text('BANK',430,245,3,[150,165,190]); text(bank,430,275,4,[255,255,255]);
  text(`INVENTORY ${inventory}`,55,350,3,[150,165,190]); text('XP',55,390,3,[150,165,190]);
  const progress=Math.max(0,Math.min(1,xp/Math.max(1,xpForNext))); rect(55,425,790,22,[35,48,72]); rect(55,425,Math.floor(790*progress),22,[88,101,242]); text(`${xp} / ${xpForNext}`,55,460,2,[220,228,245]);
...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const image = cardImage(interaction.user.username, user.level, user.xp, xpForNext, user.wallet.toLocaleString(), user.bank.toLocaleString(), inventory.length);
    const attachment = new AttachmentBuilder(image, { name: 'wimply-profile.png' });
    const progress = Math.max(0, Math.min(100, (user.xp / Math.max(1, xpForNext)) * 100));
    const embed = (await import('discord.js')).EmbedBuilder.from({
      color: 0x5865f2,
      title: STYLE.title('👤', `${interaction.user.username}'s Profile`),
      description: `〢 **Level ${user.level}** • **${Math.floor(progress)}% XP**\n〢 ${formatCurrency(user.wallet, config.currencyEmoji)} wallet • ${formatCurrency(user.bank, config.currencyEmoji)} bank\n\n${STYLE.stamp}`,
      image: { url: 'attachment://wimply-profile.png' },
      footer: { text: STYLE.brand },
