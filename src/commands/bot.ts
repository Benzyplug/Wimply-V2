...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    const botUser = await interaction.client.user.fetch(true); const avatar = botUser.displayAvatarURL({ size: 1024 }); const banner = botUser.bannerURL({ size: 2048 }) ?? avatar; const commands = interaction.client.commands?.size ?? 0; const ownerId = process.env.OWNER_DISCORD_ID; const ownerTag = ownerId ? `<@${ownerId}>` : 'ẞ€ÑZ¥'; const uptime = Math.floor(process.uptime()); const days = Math.floor(uptime / 86400); const hours = Math.floor((uptime % 86400) / 3600); const minutes = Math.floor((uptime % 3600) / 60); const seconds = uptime % 60;
    const embed = new EmbedBuilder().setColor(0x5865f2).setTitle('🤖 WIMPLY BOT').setDescription(`**${STYLE.version}**\nEconomy • Casino • Inventory • XP • Profiles\n\n👑 **Creator:** ${ownerTag}`).setThumbnail(avatar).addFields(
    ...
    ).setImage(banner).setFooter({ text: STYLE.brand }).setTimestamp();
