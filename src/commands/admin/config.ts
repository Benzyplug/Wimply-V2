...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) => sub.setName('currency').setDescription('Set currency display settings')
    ...
    .addSubcommand((sub) => sub.setName('cooldown').setDescription('Update an economy cooldown in seconds')
      ...
      .addIntegerOption((option) => option.setName('seconds').setDescription('Cooldown in seconds (use 60 for 1 minute)').setRequired(true).setMinValue(0)))
    .addSubcommand((sub) => sub.setName('interest').setDescription('Set the bank interest rate')
      .addNumberOption((option) => option.setName('rate').setDescription('Interest rate as a decimal').setRequired(true)))
    .addSubcommandGroup((group) => group.setName('reaction').setDescription('Configure automatic message reactions')
      .addSubcommand((sub) => sub.setName('add').setDescription('Add an automatic reaction rule')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (!interaction.guildId) { await interaction.reply({ embeds: [createSuccessEmbed('Server Only', 'This command must be used in a guild.')] }); return; }
    ...
    if (group === 'reaction') {
      if (subcommand === 'add') {
        ...
        if (!trigger || !emoji) throw new AppError('Trigger and emoji are required.');
        const rule = await addReactionRule(interaction.guildId, { trigger, emoji, channelId: channel.id });
        await interaction.editReply({ embeds: [createSuccessEmbed('Reaction Rule Added', `╭〔 🤖 AUTOMATION 〕╮\n〢 **ID:** \`${rule.id}\`\n〢 **Trigger:** ${trigger}\n〢 **Emoji:** ${emoji}\n〢 **Channel:** <#${channel.id}>`)] }); return;
      }
      if (subcommand === 'list') {
        const rules = await getReactionRules(interaction.guildId);
        await interaction.editReply({ embeds: [createSuccessEmbed('Reaction Rules', rules.length ? rules.map(rule => `〢 \`${rule.id}\` • ${rule.emoji} • **${rule.trigger}** • <#${rule.channelId}>`).join('\n') : '〢 No automation rules are configured yet.')] }); return;
      }
    ...
    if (subcommand === 'currency') {
      ...
      updates.currencyName = name; updates.currencyEmoji = emoji;
    } else if (subcommand === 'cooldown') {
      if (!type || seconds === null || seconds < 0) throw new AppError('Choose a valid cooldown and a non-negative number of seconds.');
