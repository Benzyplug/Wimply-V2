...
function levelStart(level: number) {
...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addStringOption(o =>
      o.setName('action')
        .setDescription('Administrator action')
        .setRequired(false)
        .addChoices(
          { name: 'Set XP', value: 'set' },
          { name: 'Add XP', value: 'add' },
          { name: 'Remove XP', value: 'remove' },
          { name: 'Set Level', value: 'level' },
          { name: 'Enable level-up notices', value: 'notify-on' },
          { name: 'Disable level-up notices', value: 'notify-off' },
          { name: 'Change level-up message', value: 'message' }
        )
    )
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    // Ensure the guild configuration exists before changing notification settings.
