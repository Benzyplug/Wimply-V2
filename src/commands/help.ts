...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const categories = [
      ...
      { title: '⚙️ SERVER SETTINGS', names: ['config','dashboard','economy','item'] },
      { title: '🤖 WIMPLY', names: ['bot','owner','ping','help'] }
    ];
    const sections = categories.map(category => { const available = category.names.filter(name => commands.includes(name)); return `╭─〔 ${category.title} 〕─╮\n〢 ${available.map(name => `**/${name}**`).join(' • ') || 'No commands loaded.'}\n╰─〔 ${available.length} command${available.length === 1 ? '' : 's'} 〕─╯`; }).join('\n\n');
    const embed = createDefaultEmbed().setTitle('╭─〔 📖 WIMPLY COMMAND CENTER 〕─╮').setDescription(`〢 **${commands.length} unique commands loaded**\n〢 **Slash:** \`/command\`\n〢 **Prefix:** \`#command\` or \`!command\`\n〢 No duplicate entries. No moderation tools.\n\n${sections}\n\n╰─〔 ⚡ Wimply V2.0 • Made by Benzy ⚡ 〕─╯`).setThumbnail(interaction.client.user.displayAvatarURL({ size: 256 }));
