...
import { getWimplyLogo, STYLE } from '../utils/presentation.js';
...
function categoryText(commands: string[]) {
  return categories.map(([title, names]) => {
    const available = names.filter(name => commands.includes(name));
    if (!available.length) return '';
    return `**${title}**\n${available.map(name => `\`${aliases[name] ? `#${aliases[name]}` : `#${name}`}\`  **/${name}**`).join('\n')}`;
  }).filter(Boolean).join('\n\n');
}
...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0x5865f2)
      .setTitle(`${getWimplyLogo(interaction.guild)} WIMPLY COMMAND CENTER`)
      .setDescription(`**${STYLE.version}**  •  **${commands.length} commands**\n\`/\` slash  •  \`#\` prefix  •  \`!\` prefix\n\n${categoryText(commands)}\n\n**Quick start**\n\`#balance\`  ·  \`#daily\`  ·  \`#work\`  ·  \`#pay @user 1000\`\n\`#mines 1000 5\`  ·  \`#aviator 1000\`  ·  \`#ccr 1000\`  ·  \`#xp\`  ·  \`#level\``)
      .setTimestamp();
