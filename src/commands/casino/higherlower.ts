...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (existing) {
      ...
      await interaction.reply({ embeds: [createDefaultEmbed().setTitle('🎯 HIGHER / LOWER — RESUMED').setDescription(`Your unfinished season is still active.\n\n〢 Current number: **${existing.current}**\n〢 Multiplier: **${existing.multiplier.toFixed(2)}x**\n〢 Moves: **${existing.moves}**\n〢 Current value: **${formatCurrency(existing.bet * BigInt(Math.round(existing.multiplier * 100)) / 100n, currency.currencyEmoji)}**\n\nYour previous progress has been restored automatically.`)], components: [buttons(existing.userId + ':' + existing.id, existing.moves)] });
      return;
    }
    const bet = parsePositiveAmount(amount);
    await chargeGame(interaction.user.id, interaction.guildId, bet, 'Higher/Lower');
    const currency = await getGameCurrency(interaction.guildId); const id = createGameSessionId(interaction.user.id); const current = Math.floor(Math.random() * 90) + 5;
