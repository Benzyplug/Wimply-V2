...
const BOT_VERSION = 'Wimply V3.0.0';
...
export function getBotBanner(clientUser: ClientUserLike): string { return clientUser.bannerURL?.({ size: 2048 }) ?? clientUser.displayAvatarURL({ size: 1024 }); }
...
function cleanText(value: string | null | undefined) { return value?.replace(/[╭╮╰╯]─〔[^〕]*〕─[╮╯]/g, '').replace(/(^|\n)\s*〢\s*/g, '$1').replace(/\n{3,}/g, '\n\n').trim(); }
function isCasinoFinalTitle(title: string | null | undefined) { if (!title) return false; const t = title.toUpperCase(); return t.includes('CASHED OUT') || t.includes('• HIT') || t.includes('• LOST') || t.includes('ROAD CLEARED') || t.includes('GAME OVER') || t.includes('BLACKJACK •') || t.includes('COINFLIP •') || t.includes('DICE •') || t.includes('SLOT •') || t.includes('SNAIL GARDEN •'); }
...
export function decorateEmbed(input: unknown, clientUser?: ClientUserLike | null) {
  ...
  if (!embed.data.author && clientUser) embed.setAuthor({ name: clientUser.username ?? 'Wimply', iconURL: clientUser.displayAvatarURL({ size: 128 }) });
