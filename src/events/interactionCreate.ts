...
const footer = 'Wimply V2.0 • Official Bot • Made by Benzy ⚡〢';
...
function withPolishedResponses<T extends Interaction>(interaction: T): T {
  ...
  return new Proxy(interaction, {
    get(target, property, receiver) {
      if (property === 'reply' || property === 'editReply' || property === 'followUp' || property === 'update') return (payload: unknown) => (target as any)[property](polishPayload(payload as any, clientUser));
...
async function handleMiniGameButton(interaction: ButtonInteraction) {
  ...
  if (parts[0] === 'hl') {
    ...
    if (!correct) { higherLowerGames.delete(id); await interaction.update({ embeds: [gameEmbed('╭─〔 💥 HIGHER / LOWER LOST 〕─╮', `〢 **BOOM!**\n〢 Previous: **${game.current}**\n〢 Next: **${next}**\n〢 Bet lost: **${formatCurrency(game.bet, '🪙')}**\n\n╰─〔 Better luck next run 〕─╯`)], components: [] }); return true; }
    game.current = next;
  ...
  if (parts[0] === 'mines') {
    ...
    if (game.mines.has(index)) {
      const finalBoard = new Set<number>(Array.from({ length: 20 }, (_, i) => i));
      minesGames.delete(id);
      await interaction.update({ embeds: [gameEmbed('╭─〔 💣 MINES HIT 〕─╮', `〢 **BOOM!** 💥\n〢 You hit a mine at tile **${index + 1}**.\n〢 Mines: **${game.mineCount}**\n〢 Gems found: **${game.revealed.size - 1}**\n〢 Bet lost: **${formatCurrency(game.bet, '🪙')}**\n\n╰─〔 🧩 Final board revealed 〕─╯`)], components: minesRows(id, finalBoard, game.mines, false) });
      return true;
    }

    game.multiplier = 1 + game.revealed.size * (0.18 + game.mineCount * 0.045);
  ...
  async execute(interaction: Interaction) {
    try {
      ...
      if (interaction.isChatInputCommand()) {
        ...
        if (!command) { log.warn(`Command not found: ${interaction.commandName}`, 'Interaction'); return; }
        ...
        if (interaction.guildId && (interaction.replied || interaction.deferred)) {
          try { const response = await interaction.fetchReply(); await reactToMatchingMessage(response, interaction.guildId, interaction.channelId, `/${interaction.commandName}`); }
          catch (error) { log.warn(`Unable to apply response reactions for /${interaction.commandName}: ${error instanceof Error ? error.message : String(error)}`, 'Reaction'); }
        }
      }
    } catch (error) { await handleInteractionError(interaction as Parameters<typeof handleInteractionError>[0], error); }
  }
};
