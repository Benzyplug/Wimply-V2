...
async function handleMiniGameButton(interaction: ButtonInteraction) {
  ...
  if (parts[0] === 'hl') {
    ...
    if (!game || game.userId !== interaction.user.id) { await interaction.reply({ embeds: [gameEmbed('╭─〔 ⏳ GAME EXPIRED 〕─╮', '〢 This Higher/Lower session is no longer active.')] }); return true; }
    if (action === 'cashout') {
