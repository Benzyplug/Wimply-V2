...
async function handleMiniGameButton(interaction: ButtonInteraction) {
  ...
  if (parts[0] === 'hl') {
    ...
    if (!game || game.userId !== interaction.user.id) { await interaction.reply({ embeds: [warningEmbed('╭─〔 ⏳ GAME EXPIRED 〕─╮', '〢 This Higher/Lower session is no longer active.')], ephemeral: true }); return true; }
    if (action === 'cashout') { if (game.moves < 1) { await interaction.reply({ embeds: [warningEmbed('╭─〔 ⚠️ CASH OUT BLOCKED 〕─╮', '〢 Make at least **1 prediction** before cashing out.')], ephemeral: true }); return true; } const payout = game.bet * BigInt(Math.round(game.multiplier * 100)) / 100n; await payGame(game.userId, game.guildId, payout, 'Higher/Lower'); higherLowerGames.delete(id); await interaction.update({ embeds: [gameEmbed('╭─〔 💰 HIGHER / LOWER CASH OUT 〕─╮', `〢 You secured **${formatCurrency(payout, '🪙')}**.\n〢 Predictions: **${game.moves}**\n〢 Multiplier: **${game.multiplier.toFixed(2)}x**\n\n╰─〔 🎯 Session secured 〕─╯`)], components: [] }); return true; }
  ...
  if (parts[0] === 'mines') {
    ...
    if (!game || game.userId !== interaction.user.id) { await interaction.reply({ embeds: [warningEmbed('╭─〔 ⏳ GAME EXPIRED 〕─╮', '〢 This Mines session is no longer active.')], ephemeral: true }); return true; }
    if (action === 'cashout') { if (game.revealed.size < 1) { await interaction.reply({ embeds: [warningEmbed('╭─〔 ⚠️ CASH OUT BLOCKED 〕─╮', '〢 Reveal at least **1 safe tile** before cashing out.')], ephemeral: true }); return true; } const payout = game.bet * BigInt(Math.round(game.multiplier * 100)) / 100n; const settledBoard = new Set<number>(Array.from({ length: 25 }, (_, i) => i)); await payGame(game.userId, game.guildId, payout, 'Mines'); minesGames.delete(id); await interaction.update({ embeds: [gameEmbed('╭─〔 💰 MINES CASH OUT 〕─╮', `〢 Secured **${formatCurrency(payout, '🪙')}** 💎\n〢 Mines: **${game.mineCount}/25**\n〢 Gems found: **${game.revealed.size}**\n〢 Multiplier: **${game.multiplier.toFixed(2)}x**\n\n╰─〔 🧩 Board settled • Session secured 〕─╯`)], components: minesRows(id, settledBoard, game.mines, false) }); return true; }
