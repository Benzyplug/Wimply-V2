...
const ALIASES: Record<string, string> = {
  ...
  ccr: 'chickencrossroad',
  chicken: 'chickencrossroad',
};
...
function prefixErrorEmbed(message: Message, description: string): EmbedBuilder {
  return baseEmbed(`${getWimplyLogo(message.guild)} COMMAND INPUT`, description)
    .setImage(getBotBanner(message.client.user));
}
...
function isUserToken(value: string | undefined): boolean {
  if (!value) return false;
  return /^<@!?\d{15,22}>$/.test(value) || /^\d{15,22}$/.test(value);
}
...
function buildPrefixInteraction(
  ...
  const options = {
    ...
    getMentionable: (name: string, required = false) => {
      const value = getValue(name, required);
      return resolveUser(message, value) ?? resolveRole(message, value);
    },
  ...
  const adapter = {
    ...
    reply: async (payload: string | MessageReplyOptions) => {
      sent = await message.reply(polishPayload(payload as any, message.client.user) as any);
      deferred = false;
      return sent;
    },
    editReply: async (payload: string | MessageReplyOptions) => {
      const polished = polishPayload(payload as any, message.client.user) as any;
      sent = sent ? await sent.edit(polished) : await message.reply(polished);
      deferred = false;
      return sent;
    },
    followUp: async (payload: string | MessageReplyOptions) => {
      sent = await message.reply(polishPayload(payload as any, message.client.user) as any);
      return sent;
    },
    deleteReply: async () => {
...
function getHelpText(prefix: string, commands: string[]): string {
  ...
  return groups
    .map(([title, names]) => {
      const available = names.filter(name => commands.includes(name));
...
async function handlePrefix(message: Message): Promise<boolean> {
  ...
  if (name === 'help' || name === 'commands') {
    ...
    const reply = await message.reply({
      embeds: [baseEmbed(`${getWimplyLogo(message.guild)} WIMPLY HELP`, description)],
    });
  ...
  if (!command) {
    const reply = await message.reply({
      embeds: [
        baseEmbed(
          `${getWimplyLogo(message.guild)} UNKNOWN COMMAND`,
          `\`${prefix}${rawName}\` is not a Wimply command.\nRun \`${prefix}help\` to open help.`,
        ),
      ],
    });
    await reactToPrefixReply(message, content, reply);
  ...
  try {
    ...
    if (isInputError) {
      const description = usage ?? rawMessage;
      const reply = await message.reply({
        embeds: [prefixErrorEmbed(message, description)],
      });
      await reactToPrefixReply(message, content, reply);
      return true;
    }
    ...
    if (state) {
      ...
      const reply = await message.reply({
        embeds: [prefixErrorEmbed(message, `Unable to process **${commandName}**.`)],
      });
      await reactToPrefixReply(message, content, reply);
    }
    ...
    try {
      ...
      if ((xpCooldown.get(key) ?? 0) <= now) {
        ...
        const updated = await awardXp(
          message.author.id,
          message.guildId,
          before.config.xpPerMessage || 5,
        );

        if (
          updated &&
          updated.level > oldLevel &&
          before.config.levelUpEnabled
        ) {
          const text = before.config.levelUpMessage
            .replace(/\{user\}/g, `<@${message.author.id}>`)
            .replace(/\{level\}/g, String(updated.level));

          const notification = baseEmbed(
            `${getWimplyLogo(message.guild)} LEVEL UP`,
            text,
          ).setImage(getBotBanner(message.client.user));
      ...
      log.error(
        'Failed to process message/prefix automation',
        'MessageCreate',
        error,
      );
    }
