...
function prefixErrorEmbed(message: Message, description: string) {
  const logo = getWimplyLogo(message.guild);
  return baseEmbed(`${logo} COMMAND INPUT`, description).setImage(getBotBanner(message.client.user));
}
...
function buildPrefixInteraction(message: Message, command: Command, tokens: string[]): PrefixAdapterState {
  ...
  const getValue = (name: string, required = false) => {
    ...
    if (required && value === undefined) {
      const gamble = GAMBLE_USAGE[command.data.toJSON().name];
      if (gamble) throw new Error(`❌ ${gamble}`);
  ...
  const options = {
    getString: (name: string, required = false) => {
      ...
      if (command.data.toJSON().name === 'coinflip' && name === 'choice') {
  ...
  const adapter = {
    user: message.author, member: message.member, guild: message.guild, guildId: message.guildId, channel: message.channel, client: message.client,
    memberPermissions: message.member?.permissions ?? null, options,
...
async function handlePrefix(message: Message): Promise<boolean> {
  ...
  if (name === 'help' || name === 'commands') {
    ...
    const groups = [
      ...
      ['⚙️ SERVER', ['config','dashboard','economy','item','xp','level']],
      ['🤖 WIMPLY', ['bot','owner','ping','help']]
    ] as const;
    const sections = groups.map(([title, names]) => {
      const available = names.filter(command => commands.includes(command));
      return available.length ? `**${title}**\n${available.map(command => `\`${prefix}${Object.entries(ALIASES).find(([, target]) => target === command)?.[0] ?? command}\`  **/${command}**`).join('\n')}` : '';
    }).filter(Boolean).join('\n\n');
    const reply = await message.reply({ embeds: [baseEmbed(`${getWimplyLogo(message.guild)} WIMPLY HELP`, `**${commands.length} commands**  •  \`/\` slash  •  \`#\` or \`!\` prefix\n\n${sections}\n\n**Quick start**\n\`#balance\`  ·  \`#daily\`  ·  \`#work\`  ·  \`#pay @user 1000\`\n\`#mines 1000 5\`  ·  \`#aviator 1000\`  ·  \`#ccr 1000\`)] });
    await reactToPrefixReply(message, content, reply); return true;
  }

  const command = message.client.commands?.get(name);
  if (!command) {
    const reply = await message.reply({ embeds: [baseEmbed('❔ UNKNOWN COMMAND', `\`${prefix}${rawName}\` is not a Wimply command.\nRun \`${prefix}help\` to open help.`)] });
    await reactToPrefixReply(message, content, reply); return true;
  }
  ...
  try {
    ...
    if (state) {
      ...
      const messageText = error instanceof Error ? error.message.replace(/^❌\s*/, '') : `Unable to process ${commandName}.`;
      const usage = GAMBLE_USAGE[commandName];
      const reply = await message.reply({ embeds: [prefixErrorEmbed(message, usage ?? messageText)] });
    ...
    try {
      ...
      if ((xpCooldown.get(key) ?? 0) <= now) {
        ...
        if (updated && updated.level > oldLevel && before.config.levelUpEnabled) {
          ...
          await message.channel.send({ embeds: [notification] });
        }
      }
    } catch (error) {
