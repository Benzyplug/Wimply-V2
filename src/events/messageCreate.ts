...
const ALIASES: Record<string, string> = {
  hl: 'higherlower', higher: 'higherlower', lower: 'higherlower',
  sg: 'snailgarden', snail: 'snailgarden',
  bj: 'blackjack', cf: 'coinflip', lb: 'leaderboard', inv: 'inventory',
  bal: 'balance', p: 'profile', sh: 'shop'
};
...
function baseEmbed(title: string, description: string) { return new (requireEmbed())().setColor(0x5865f2).setTitle(title).setDescription(description).setTimestamp(); }
function requireEmbed() { return (globalThis as any).EmbedBuilder; }
...
function resolveCommandShape(command: Command, tokens: string[]) {
  const json = command.data.toJSON() as unknown as PrefixCommandJson; let options = json.options ?? []; let index = 0; let subcommand: string | undefined; let subcommandGroup: string | undefined; const first = options[index];
  ...
  const values = new Map<string, string>(); for (const option of options.filter(option => option.type >= 3)) { const value = tokens[index]; if (value !== undefined) { values.set(option.name, value); index += 1; } }
...
function buildPrefixInteraction(message: Message, command: Command, tokens: string[]): PrefixAdapterState {
  const { values, subcommand, subcommandGroup } = resolveCommandShape(command, tokens); let sent: Message | null = null; let deferred = false;
  const getValue = (name: string, required = false) => { const value = values.get(name); if (required && value === undefined) throw new Error(`Missing required argument: **${name}**`); return value; };
  const options = {
  ...
  const adapter = {
    user: message.author, member: message.member, guild: message.guild, guildId: message.guildId, channel: message.channel, client: message.client, memberPermissions: message.member?.permissions ?? null, options,
    get replied() { return sent !== null; }, get deferred() { return deferred; }, isRepliable: () => true, isChatInputCommand: () => true,
...
async function reactToPrefixReply(message: Message, triggerContent: string, reply: Message | null) { const guildId = message.guildId; if (!reply || !guildId) return; await reactToMatchingMessage(reply, guildId, message.channelId, triggerContent); }
async function handlePrefix(message: Message): Promise<boolean> {
  const content = message.content.trim(); const prefix = PREFIXES.find(candidate => content.startsWith(candidate)); if (!prefix) return false;
  const tokens = tokenize(content.slice(prefix.length).trim()); const rawName = tokens.shift()?.toLowerCase(); if (!rawName) return false;
  const name = ALIASES[rawName] ?? rawName;
  if (name === 'help' || name === 'commands') { const commands = message.client.commands ? [...message.client.commands.keys()].sort() : []; const grouped = commands.length ? commands.map(command => `• \`${prefix}${command}${ALIASES[command] ? `\` · \`${prefix}${ALIASES[command]}` : '\`'}`).join('\n') : '• No commands loaded.'; const reply = await message.reply({ embeds: [baseEmbed(STYLE.title('📖', 'Wimply Command Center'), `〢 **Prefix:** \`#\` or \`!\`\n〢 **Slash:** \`/\`\n〢 **Loaded:** **${commands.length}** commands\n\n${grouped}\n\n${STYLE.bottom('⚡', 'ẞ€ÑZ¥')}`)] }); await reactToPrefixReply(message, content, reply); return true; }
  const command = message.client.commands?.get(name);
  if (!command) { const reply = await message.reply({ embeds: [baseEmbed(STYLE.title('❔', 'Unknown Command'), `〢 \`${prefix}${rawName}\` is not a Wimply command.\n〢 Run \`${prefix}help\` to open the command center.\n\n${STYLE.bottom('🧭', 'ẞ€ÑZ¥')}`)] }); await reactToPrefixReply(message, content, reply); return true; }
  const state = buildPrefixInteraction(message, command, tokens); try { await command.execute(state.interaction); } catch (error) { await handleInteractionError(state.interaction, error); }
  await reactToPrefixReply(message, content, state.getReply()); return true;
}
export default { name: 'messageCreate', once: false, async execute(message: Message) { if (!message.guildId || message.author.bot || !message.content.trim()) return; try { await handlePrefix(message); } catch (error) { log.error('Failed to process message/prefix automation', 'MessageCreate', error); } } };
